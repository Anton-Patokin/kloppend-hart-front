...
class NAApiHelper
{
 	...
 	public  $devicelist;
 	private $client_id = "5090e81618775915840051bf";
 	private $client_secret = "KE0qQ6BvBJC3OUoVZQVQlFQDIYvs";
 	private $username = "dimi@edge.be";
	...
	public function __construct(){
		...
		$this->client = new NAApiClient(array("client_id" => $this->client_id, "client_secret" => $this->client_secret));
		$this->client->setVariable("username", $this->username);
		$this->client->setVariable("password", $this->password);
		
		$this->client->getAccessToken();
		
		$this->devicelist = $this->client->api("devicelist", "POST");
                $this->SimplifyDeviceList();
	}
	
    public function SimplifyDeviceList() 
    {
        foreach ($this->devicelist["devices"] as $d=>$device) 
        {
            $moduledetails=Array();
            foreach ($device["modules"] as $module) 
            {
                foreach ($this->devicelist["modules"] as $moduledetail) 
                {
            ...
            unset($this->devicelist["devices"][$d]["modules"]);
            $this->devicelist["devices"][$d]["modules"]=$moduledetails;
        }
        unset($this->devicelist["modules"]);
    }
    public function GetLastMeasure($device, $module=null) 
    {
    ...
    {
        $results = Array();
        foreach ($this->devicelist["devices"] as $device) 
        {
            $result = Array();
            ...
            $result["modules"][0] = array_merge($result["modules"][0], $this->GetLastMeasure($device["_id"]));
            foreach ($device["modules"] as $module) 
            {
                $addmodule = Array();
                if(isset($module["module_name"])) $addmodule["module_name"] = $module["module_name"];
                $addmodule = array_merge($addmodule, $this->GetLastMeasure($device["_id"],$module["_id"]));
