<?php
    ...
    if (isset($_GET['nid'])) {
        ...
        $url = 'http://localhost/edge/projects/kloppend-hart-antwerpen/version_002/front/application/service/place/getPlaceInfoByNid/' . $getNid;
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $placeInfo = json_decode(curl_exec($ch), true);
        curl_close($ch);

        $url = 'http://localhost/edge/projects/kloppend-hart-antwerpen/version_002/front/application/service/place/getPlaceStatsByNid/' . $getNid;
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
            <?php if (isset($placeInfo) && !empty($placeInfo)): ?>
                <?= $placeInfo['title'] ?>
                <?= $placeInfo['body'] ?>
            <?php endif ?>
        </div>
